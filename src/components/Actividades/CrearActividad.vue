<template>
  ...
  >
    <!-- Modal Content -->
    <div
      class="bg-white p-6 rounded-lg max-h-[90vh] relative overflow-auto"
...
const fetchOrigins = async () => {
  try {
    origins.value = await OrigenesService.getAllOrigins(token)
  } catch (error) {
...
const handleOriginChange = (value: string) => {
  selectedOriginId.value = value
  actividad.value.origin_id = value
}
...
const submitActivity = async () => {
  ...
  const activityData = {
    ...
    origin_id: actividad.value.origin_id, // Nuevo campo para el origen
  ...
  try {
    ...
    // Enviar la solicitud para crear la actividad
    const actividadCreada = await ActividadesService.createActivity(
      authStore.getToken,
      activityData
    )
    ...
    if (actividadCreada && actividadCreada.id) {
      ...
      const response = await ActividadesService.addInterestsToActivity(
        authStore.getToken,
        actividadCreada.id,
        interestIds
      )
