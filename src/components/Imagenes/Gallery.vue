  ...
  <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
    <div class="p-4">
      ...
      <div class="mb-4 flex justify-between">
        ...
        <select v-model="sortOrder" id="orderSelect" @change="loadImages">
      ...
      <!-- Galería de imágenes -->
      <div v-if="imageData.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 auto-rows-min">
        <div v-for="(image, index) in imageData" :key="image.id" class="relative w-full h-48 overflow-hidden rounded-lg shadow-md">
          <img :src="image.url" alt="Imagen subida" class="absolute inset-0 w-full h-full object-cover" />
          <!-- Botón de borrar -->
          <button @click="deleteImage(image.id, image.url)" class="absolute top-2 right-2 bg-red text-white p-1 rounded-lg shadow-md hover:bg-red-700">
      ...
      <div class="flex justify-between mt-4">
        <button @click="previousPage" :disabled="currentPage === 1" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded disabled:opacity-50 disabled:cursor-not-allowed">
          Anterior
        </button>
        <button @click="nextPage" :disabled="!hasMoreImages" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Siguiente
        </button>
      </div>
    </div>
...
// Estado que contiene las imágenes (URL y referencia)
...
const sortOrder = ref<'ASC' | 'DESC'>('ASC') // Orden ascendente por defecto
const hasMoreImages = ref(true)

// Cargar imágenes paginadas desde el servidor
const loadImages = async () => {
  try {
    const response = await ImagenService.getImages(authStore.getToken, limit, offset.value, sortOrder.value)
...
// Función para eliminar la imagen usando su ID y URL
const deleteImage = async (imageId: string, imageUrl: string) => {
  try {
    const fileRef = storageRef(storage, imageUrl)
    await deleteObject(fileRef)

    await ImagenService.eliminarImagenDeGaleria(authStore.getToken, imageId)

    imageData.value = imageData.value.filter((image) => image.id !== imageId)
