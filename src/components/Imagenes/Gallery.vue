    ...
    <div class="p-4">
      ...
      <div v-if="imageData.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        <div v-for="(image, index) in imageData" :key="image.id" class="relative w-full h-48 overflow-hidden rounded-lg shadow-md">
          ...
          <!-- Botón de borrar -->
          <button
            @click="deleteImage(image.id, image.url)"
...
// Cargar imágenes desde el servidor y Firebase
const loadImages = async () => {
  try {
    // Cargar imágenes desde tu servidor
...
const deleteImage = async (imageId: string, imageUrl: string) => {
  try {
    ...
    // Eliminar la imagen desde tu servidor
    const response = await ImagenService.deleteImageFromGallery(authStore.getToken, imageId);
    console.log(response)
    // Actualizar la lista de imágenes eliminando la imagen correspondiente
