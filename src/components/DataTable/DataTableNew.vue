<template>
  ...
  const rowsPerPage = ref(5) // Fixed rows per page
  ...
  watch(
    ...
    () => {
      // Optional: You can handle changes here if needed, e.g., to update other parts of the UI.
    }
  )
  ...
  function onRowsPerPageChange() {
    // Optional: This function can be used if you want to handle changes, but it won't affect rowsPerPage.
    // It only updates the selected value, which does not affect the table rows.
    selectedRowsPerPage.value = Number(selectedRowsPerPage.value)
  }
  
  function updatePagination() {
    emit('updatePagination', {
      rowsPerPage: rowsPerPage.value, // Fixed value
